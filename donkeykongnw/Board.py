...
class Board:
    def __init__(self, width, height):
        ...
        self.Buttons = [Button(pygame.image.load('Assets/start.png'), (450, 400), "start"),
        ...
        self.startbackground = pygame.image.load('Assets/buttonbackground1.jpeg')
    ...
    def CreateFireball(self, location, kongIndex):
        if len(self.Fireballs) < len(self.Enemies) * 1+1:
    ...
    def makePrincessChamber(self) -> object:
        for j in range(0, 5):
            self.map[j][9] = 1
    ...
    def redrawScreen(self, displayScreen, scoreLabel, width, height):
        ...
        if self.gameState != 1:
            ...
            if self.gameState == 0:
                # Pregame state
                displayScreen.blit(pygame.image.load('Assets/barelbreakouttext.png'), (340, 50))
            if self.gameState == 2:
                # Post game state
